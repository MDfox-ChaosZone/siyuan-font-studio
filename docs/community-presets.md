...
4. GitHub Issue 的 ZIP 附件限制按**单个文件**计算。完整方案包超过 24 MB 时，可在插件导出窗口勾选“分享至 GitHub Issue 时拆分”，逐个下载生成的 2–5 个 ZIP 分包，并把**全部分包**拖入同一个 Issue 的“字体方案压缩包”字段。每个分包小于 25 MB；不要解压、重压或只提交其中一份。完整方案包仍不得超过 100 MiB。也可将完整 ZIP 或 JSON 上传到自己的网盘或 GitHub Release 并填写 HTTPS 链接；无需登录、无需提取码的文件直链可自动检查，普通网盘分享页面需要维护者人工下载，按下文交接。
